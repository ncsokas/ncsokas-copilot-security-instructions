...
# Directories to skip entirely (comma-separated, matched as path prefixes).
# Default: .github/ — documentation and CI config rarely contain real secrets
# and frequently contain example values that trigger false positives.
# Override with EXCLUDED_DIRS="dir1,dir2" or set to "" to scan everything.
DEFAULT_EXCLUDED_DIRS=".github/"
EXCLUDED_DIR_LIST=()
IFS=',' read -ra EXCLUDED_DIR_LIST <<< "${EXCLUDED_DIRS:-$DEFAULT_EXCLUDED_DIRS}"
...
# Returns 0 (true) if the path should be excluded from scanning
is_excluded_path() {
  local filepath="$1"
  for dir in "${EXCLUDED_DIR_LIST[@]}"; do
    dir=$(printf '%s' "$dir" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
    [[ -z "$dir" ]] && continue
    # Normalise: ensure the prefix ends with / for exact dir matching
    [[ "$dir" != */ ]] && dir="${dir}/"
...
scan_file() {
  ...
  # Skip excluded directories (e.g. .github/ docs, demo/ intentional bad-code dirs)
  ...
  for entry in "${PATTERNS[@]}"; do
    ...
    while IFS=: read -r line_num matched_line; do
      ...
      # Skip if the "value" portion looks like a function/method call or variable
      # reference rather than a literal secret (e.g. `token = crypto.createHash(`)
      if printf '%s\n' "$match" | grep -qiE '[=:][[:space:]]*(crypto\.|Math\.|Buffer\.|require\(|process\.|new |getenv|os\.environ|config\.|settings\.)'; then
        continue
      fi
